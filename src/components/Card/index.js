...
const Card = ({city}) => {
  ...
  return (
    <div className={` ${s[city.weather[0].description.split(" ").join("_")]} flex relative flex-col shadow-lg shadow-gray-500 items-center w-60 h-80`}>
      ...
      <div className='flex flex-col items-center justify-center'>
          <p className='text-white py-1 px-4 font-semibold rounded-md bg-gray-900 bg-opacity-50' >Min Temp : {city.main.temp_min}</p>
          <p className='text-white py-1 mt-4 font-semibold px-4 rounded-md bg-gray-900 bg-opacity-50' >Max Temp : {city.main.temp_max}</p>
      </div>
    </div>
  )
}
